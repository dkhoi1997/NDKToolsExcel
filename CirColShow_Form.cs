...
{
    public partial class CirColShow_Form : Form
    {
        double Rb; double Rbt; double Eb; double Rs; double Rsc; double Rsw; double Es; int nsec; string shape; double umin; double umax;
        public CirColShow_Form(double re_D, double re_Rb, double re_Rbt, double re_Eb, double re_Rs, double re_Rsc,
            double re_Rsw, double re_Es, double re_L, int re_abv, int re_nsec, string re_dmain, int re_nx, string re_dstir,
    ...
    string Xlabel, string Ylabel, bool swap, int[] maxvalue) // Hàm format chart
        ...
        public void SectionChart(Chart chart, ChartArea objectchart) // Hàm chart hiện thị tiết diện thép
        {
            ...
            int sumn = Convert.ToInt32(sumTextBox.Text);
            ...
            for (int k = 0; k <= 30; k++)
            {
                double x = 0 + D / 2 * Math.Cos(k * 2 * Math.PI / 30);
                double y = 0 + D / 2 * Math.Sin(k * 2 * Math.PI / 30);
                chart.Series[0].Points.AddXY(x, y);
            }

            chart.Series[0].Color = Color.Black;
            double D_cen = D - 2 * abv - 2 * dstir - dmain;
            double[,] rebar_element = new double[sumn, 2];
            for (int i = 0; i < sumn; i++)
            {
                rebar_element[i, 0] = Math.Sin(2 * Math.PI / sumn * i) * (D_cen / 2);
                rebar_element[i, 1] = Math.Cos(2 * Math.PI / sumn * i) * (D_cen / 2);
            }
            chart.Series.Add("srs2");
            chart.Series[1].IsVisibleInLegend = false;
            chart.Series[1].ChartArea = "ChartArea1";
            ...
            for (int j = 0; j <= rebar_element.GetUpperBound(0); j++)
            {
                chart.Series[1].Points.AddXY(rebar_element[j, 0], rebar_element[j, 1]);
            }
            float h = (float)((float)100 * 420 / 180);
            if (h > 100) h = 100;
            float w = (float)((float)100 * 180 / 420);
            if (w > 100) w = 100;
            float pos_X = (100 - w) / 2;
            float pos_Y = (100 - h) / 2;
            ...
            objectchart.AxisX.Maximum = D / 2;
            objectchart.AxisX.Minimum = -D / 2;
            objectchart.AxisY.Maximum = D / 2;
            objectchart.AxisY.Minimum = -D / 2;
            ...
            ReCalculate(double Rb, double Rbt, double Eb, double Rs, double Rsc, double Rsw, double Es, int nsec) // Hàm tính toán lại để show kết quả
        {
            ...
            double D = Convert.ToDouble(DTextBox.Text) / 1000;
            int sum_rebar = Convert.ToInt32(sumTextBox.Text);
            int dmain = Convert.ToInt32(dmainTextBox.Text);
            int dstir = Convert.ToInt32(dstirTextBox.Text);
            ...
            var instance = new NDKFunction();
            // PMxy
            int Mx_up = instance.CircleUpperMoment(P, Mx, My, D, L, Eb, k).Item1;
            int My_up = instance.CircleUpperMoment(P, Mx, My, D, L, Eb, k).Item2;
            double[,] ver_check = instance.IDSurfaceCircle(D * 1000, Eb, Es, abv, Rb, Rs, Rsc, sum_rebar, dmain, dstir, nsec).Item1;
            double[,] hoz_check = instance.IDSurfaceCircle(D * 1000, Eb, Es, abv, Rb, Rs, Rsc, sum_rebar, dmain, dstir, nsec).Item2;
            ...
            // PQ
            double D_square = Math.Sqrt(Math.PI * Math.Pow(D, 2) / 4);
            int Qnx = instance.StirrupRectangleCheck(D_square, D_square, Pq, Qx, Qy, dmain, dstir, nstir, sw, abv, Rb, Rbt, Rsw).Item1;
            int Qny = instance.StirrupRectangleCheck(D_square, D_square, Pq, Qx, Qy, dmain, dstir, nstir, sw, abv, Rb, Rbt, Rsw).Item2;
            object DCs = instance.StirrupRectangleCheck(D_square, D_square, Pq, Qx, Qy, dmain, dstir, nstir, sw, abv, Rb, Rbt, Rsw).Item3;
            // ved
            double ved = 0;
            if (Ped > 0)
            {
                ...
                ved = Math.Round(Ped / (D_square * D_square * fcd), 2);
            }

            return (out_PMxy, out_MxMy, Mx_up, My_up, Pnxy, Mnxy, DC, Qnx, Qny, DCs, ved);
        }

        public void FillTextBox1(double D, int nx, int dmain, int dstir, int sw,
            ...
            string shape, string cad_label, string etabs_label, string story) // Hàm show TextBox sẵn có
        {
            ...
            double muy = Math.Round(Asc / (Math.PI * Math.Pow(D, 2) / 4) * 100, 2);
        ...
        public void ShowMemberCircle() // Gom hàm vào 1 function
        {
        ...
        public (object[], object[], object[], object[]) SendBack() // Trả kết quả về Main Form
        {
            ...
            double D = (double)Convert.ToInt32(DTextBox.Text) / 1000;
            int abv = Convert.ToInt32(abvTextBox.Text);
            ...
            send_data2[5] = Convert.ToString(instance.SimplifiedCircleID(P, Mx_up, My_up, D, abv, Rb, Rs, umin, umax));
            send_data2[6] = sumTextBox.Text;
            send_data2[7] = null;
            send_data2[8] = send_data2[6] + "Ø" + dmainTextBox.Text;
